...
p5.prototype.registerMethod("init", function p5PlayInit() {
    ...
    class Sprite {
            ...
            if (shape == "box") {
                ...
                if (this.originMode == "center") {
                    for (let i = 0; i < vecs.length; i++) {
                        let vec = vecs[i];
                        vecs[i] = new pl.Vec2(
                            ((vec.x - this._hw - min.x) * this.tileSize) /
                                plScale,
                            ((vec.y - this._hh - min.y) * this.tileSize) /
                                plScale
                        );
                    }
                } else {
                    ...
                    for (let i = 0; i < vecs.length; i++) {
                        let vec = vecs[i];
                        vecs[i] = new pl.Vec2(
                            (vec.x * this.tileSize) / plScale,
                            (vec.y * this.tileSize) / plScale
                        );
                    }
                }
                if (shape == "polygon") {
        ...
        set hw(val) {
            throw new Error(
                "Unable to change the value of halfWidth directly, change the sprite's width instead"
            );
        }
        /**
         * The width of the sprite.
        ...
        set hh(val) {
            throw new Error(
                "Unable to change the value of halfHeight directly, change the sprite's height instead"
            );
        }
        /**
         * The height of the sprite.
        ...
        set r(val) {
            throw new Error(
                "Unable to change the value of radius directly, change the sprite's diameter instead"
            );
        }
        ...
        _display() {
            let x =
                this.p.width * 0.5 -
                this.p.world.origin.x +
                this.x * this.tileSize;
            let y =
                this.p.height * 0.5 -
                this.p.world.origin.y +
        ...
        move(destX, destY, speed) {
            ...
            let dist = Math.max(
                Math.abs(this.x - this.dest.x),
                Math.abs(this.y - this.dest.y)
            );
        ...
        rotate(angle, speed) {
            if (!angle) {
                throw new Error(
                    "angle must be a number greater or less than zero"
                );
            }
        ...
        _changeAni(label) {
            ...
            if (!ani) {
                this.p.noLoop();
                throw new Error(
                    "changeAnimation error: no animation named " + label
                );
            }
            this._animation = ani;
        ...
        constructor() {
            ...
            // SpriteSheet mode
                ...
                if (args[0] instanceof p5.Image || typeof args[0] == "string") {
                    if (args.length >= 3) {
                        throw new Error(
                            "SpriteAnimation error: the name of animation should go first"
                        );
                    }
                    sheet = args[0];
                    atlas = args[1];
                ...
                function _generateSheetFrames() {
                    ...
                    if (!w || !h) {
                        if (tileSize) {
                            ...
                            _this.spriteSheet.width < _this.spriteSheet.height
                        ) {
    ...
    class Group extends Array {
        ...
        moveTowards(x, y, tracking) {
            ...
            for (let s of this) {
                if (s.distCentroid === undefined)
                    this.resetDistancesFromCentroid();
                let dest = {
        ...
        remove(item) {
            ...
            if (idx !== undefined) {
                ...
                let gIdx = this[idx].groups.findIndex(
                    (g) => g.idNum == this.idNum
                );
     ...
     */
    p5.prototype.updateSprites = function (
        timeStep,
        velocityIterations,
        positionIterations
    ) {
        ...
        pInst.world.queryAABB(aabb, (fxt) => {
            if (!fxt.getBody().isStatic()) {
                if (
                    fxt
                        .getShape()
                        .testPoint(fxt.getBody().getTransform(), convertedPoint)
                ) {
                    fxts.push(fxt);
                }
            }
    ...
    p5.prototype.drawSprite = function (sprite) {
        if (pInst.frameCount == 1)
            console.warn(
                "drawSprite() is deprecated, use sprite.draw() instead."
            );
        sprite.draw();
    };
    ...
    p5.prototype.drawSprites = function (group) {
        if (pInst.frameCount == 1)
            console.warn(
                "drawSprites() is deprecated, use group.draw() instead."
            );
        group ??= pInst.allSprites;
        group.draw();
    };
    ...
    p5.prototype.play = (sound) => {
        if (!sound.play)
            throw new Error(
                "Tried to play a sound but the sound is not a sound object: " +
                    sound
            );
    ...
    this._onmouseup = function (e) {
        ...
        if (this.p5play.mouseSprite) {
            if (this.p5play.mouseSprite.mouse.hover > 1) {
                if (
                    this.p5play.mouseSprite.mouse[btn] >=
                    this.mouse.holdThreshold
                ) {
    ...
    class KeyBoard extends InputDevice {
        ...
        ac(inp) {
            ...
            if (!isNaN(inp)) {
                ...
                throw new Error(
                    "Use key names with the keyboard input functions, not key codes!"
                );
            }
...
p5.prototype.registerMethod("post", function p5playPostDraw() {
    ...
    if (this.p5play.mouseTracking) {
        ...
        let uiSprites = p5.prototype.getSpritesAt(
            this.camera.mouse.x,
            this.camera.mouse.y,
            this.allSprites,
            false
